...
@Gtk.Template(resource_path='/org/gnome/Jokosher/gtk/addinstrumentdialog.ui')
class AddInstrumentDialog(Gtk.Window):
    __gtype_name__ = 'AddInstrumentDialog'
    """
    ...
    def __init__(self, project, parent, instr=None):
        ...
        self.model = Gtk.ListStore(str, str, GdkPixbuf.Pixbuf)

        for instr in Instrument.getInstruments():
        ...
        self.tree.set_item_orientation(Gtk.Orientation.VERTICAL)
        if self.instr:
            self.tree.set_selection_mode(Gtk.SelectionMode.SINGLE)
        else:
            self.tree.set_selection_mode(Gtk.SelectionMode.MULTIPLE)

        self.tree.set_item_width(90)
        self.tree.set_size_request(72, -1)

        width = int(Settings.get_settings().general["addinstrumentwindowwidth"])
        height = int(Settings.get_settings().general["addinstrumentwindowheight"])
